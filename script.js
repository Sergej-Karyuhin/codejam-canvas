...
function draw_32x32() {
  const size = 16;
  for (let i = 0; i < matrix_32x32.length; i++) {
    for (let j = 0; j < matrix_32x32.length; j++) {
      ctx.fillStyle = 'rgb(' + matrix_32x32[i][j].toString() + ')';
      ctx.fillRect( (i * size), (j * size), size, size );
    }
  }
}

canvas_32x32.addEventListener('click', function() {
  draw_32x32();
  canvas_current.forEach(item => item.classList.remove('active'));
  canvas_current[1].classList.add('active');
});
...
canvas_img.addEventListener('click', function() {
  draw_img();
  canvas_current.forEach(item => item.classList.remove('active'));
  canvas_current[2].classList.add('active');
});
